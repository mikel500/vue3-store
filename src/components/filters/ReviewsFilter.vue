<template>
  <div>
    ...
    <ol v-if="showReviewFilter">
      ...
      >
        <button
          v-if="selectedOption === star"
          class="w-full flex rounded-md text-base font-medium mt-2 mb-2 bg-gray-700"
      ...
      <li class="flex justify-end">
        <button @click="cleanReviewsFilter()">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-6 w-6"
            fill="none"
...
export default {
  ...
  setup (_, context) {
    ...
    const showReviewFilter = ref(true)
    const selectedOption = ref(null)
    const { stars } = useProductFilters()

    const reverseKeys = (n) => Array.from(Array(n).keys()).reverse()

    const getOutlineStar = (n) => 5 - n

    const getFilterValue = (newVal) => {
      selectedOption.value = newVal
      context.emit("reviewsFilterUpdated", { value: newVal, filter: 'stars' })
    }

    const cleanReviewsFilter = () => {
      selectedOption.value = null
      context.emit('cleanReviewsFilter', 'reviews')
    }
